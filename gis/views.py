...
def location_search(request):
    ...
    # url = f"http://nominatim:8080/search?q={search_text}&format=jso
    # n&polygon_geojson=1&addressdetails=1&accept-language=en"
...
def route(request):
    ...
    # url = f"http://maarga-container:5000/route/v1/driving/{source_lon},{sour
    # ce_lat};{destination_lon},{destination_lat}?overview=false&alternatives=true&steps=true"
    url = (
        "http://maarga-container:5000/route/v1/driving/"
...
@api_view(["GET"])
def distance_and_duration(request):
    source_lon = request.query_params.get("source_lon")
    source_lat = request.query_params.get("source_lat")
    destination_lon = request.query_params.get("destination_lon")
    destination_lat = request.query_params.get("destination_lat")
