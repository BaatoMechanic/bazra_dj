...
def is_django_class_view(url: Any) -> bool:
    ...
    return (
        cls is not None
        and not re.search("WrappedAPIView", str(url.callback))
...
def is_rest_model_viewset(url: URLPattern) -> bool:
    ...
    # )
    ad = bool(set(REST_ACTIONS).intersection(dir(cls)) and cls.queryset.model)
    return ad
...
def is_websocket_consumer(url: URLPattern) -> bool:
    ...
    return (
        not getattr(url.callback, "cls", None)
        and not getattr(url.callback, "view_class", None)
        and getattr(url.callback, "__wrapped__")
        and issubclass(url.callback.__wrapped__, WebsocketConsumer)
    )
...
def get_django_class_view_permission(url: str) -> List[Dict[str, Optional[str]]]:
    ...
    http_methods = ["get", "post", "put", "patch", "delete"]
    permissions = [
        {"method": method, "detail": get_detail(get_django_class_view_action(url, method))}
...
def get_rest_decorated_view_permission(url: URLPattern) -> List[Dict[str, str]]:
    ...
    """
    permissions = [
        {"method": method, "detail": get_detail(get_rest_decorated_view_action(url))}
...
def get_rest_model_viewset_permission(url: Any) -> List[Dict[str, Any]]:
    ...
    list_action_map: Dict[str, str] = {"list": "get", "create": "post"}
    ...
    if re.match(r".*?-detail", url.name):
        for action_name in detail_action_map:
            ...
            if action:
                permissions.append({"method": detail_action_map[action_name], "detail": get_detail(action)})
    elif re.match(r".*?-list", url.name):
        for action_name in list_action_map:
            action = get_rest_model_viewset_action(url, action_name)
            if action:
                permissions.append({"method": list_action_map[action_name], "detail": get_detail(action)})
    else:
...
def get_rest_non_model_viewset_permission(url):
    ...
    try:
        for method in list(action.bind_to_methods):
            permissions.append({"method": method, "detail": remove_special_characters(action.__doc__ or "")})
    except AttributeError:
...
def get_url_meta(url: URLPattern) -> Dict[str, str]:
    ...
    return {"name": name, "url": url_pattern, "permissions": permissions}
...
def attach_permissions(role_name, permissions):
    ...
    for permission in permissions:
        try:
            ...
            permissions_objs.append(Permission.objects.get(name=permission[0], method=permission[1]))
